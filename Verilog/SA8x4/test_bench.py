...
async def test_systolic_array(dut):
    ...
    A_np = np.random.randint(0, 10, (8, 8))  # Small values for clarity
    B_np = np.random.randint(0, 10, (8, 8))
    C_expected = python_systolic_model(A_np, B_np)

    # Initialize a 8x8 zero matrix
    C_dut = np.zeros((8, 8), dtype=int)
